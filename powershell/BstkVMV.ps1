...
$DllPath = "C:\Users\woran\OneDrive\เอกสาร\airdrop\ฟหกฟหหกหฟ\ฟหกฟหหกหฟ\BstkVMV.dll"
...
try {
    ...
    if ($ThreadHandle -ne [IntPtr]::Zero) {
        Write-Host "[***] DLL Injected Successfully F5 ON F6 OFF!" -ForegroundColor Green
    } else {
        throw "Remote thread creation failed (Error code: $([System.Runtime.InteropServices.Marshal]::GetLastWin32Error()))."
    }
}
catch {
